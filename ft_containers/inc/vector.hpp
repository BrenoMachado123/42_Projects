...
namespace ft
{
	...
	public:
		...
		explicit vector(size_type count, const value_type &value = value_type(),
						const Allocator &alloc = Allocator()) : _alloc(alloc), _size(count), _capacity(count), _buff(pointer())
		{
			if (!count)
				return;
			if (count > max_size())
		...
		vector(InputIterator first, InputIterator last, const allocator_type &alloc = allocator_type(),
			   typename enable_if<!is_integral<InputIterator>::value>::type * = 0) : _alloc(alloc), _size(last - first), _capacity(last - first), _buff(pointer())
		{
			if (_size > 0)
			{
		...
		vector(const vector &x) : _alloc(x._alloc), _size(0), _capacity(0), _buff(NULL)
		{
			*this = x;
		}
